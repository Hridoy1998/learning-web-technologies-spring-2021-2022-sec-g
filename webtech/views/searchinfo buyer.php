...
<h2 align="center" style="color:crimson"><b>Buyer</b></h2>
	<table border="1" align="center">
		<tr><th><h2 style="color:cornsilk">ID</h2></th>
		...
		<?php
		  ...
		  $con = getConnection();
		  $sql = "SELECT * from user where (username ='{$value}'or id='{$value}' or email='{$value}') and type='Buyer'";
		  $result = $con->query($sql);
		  if($result !== false && $result->num_rows > 0){
			echo('<h2 style="color:black;">Found</h2>');
		  while($row = $result->fetch_assoc()) {
		?>
		<tr><td style="color:cornsilk"><?=$row["id"]?></td>
